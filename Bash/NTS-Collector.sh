...
if [ "$EUID" -ne 0 ]; then
  echo "${RED}[!] Por favor, execute este script como root.${NRED}"
  exit 1
fi

# Solicita a senha de root para instalar pacotes sem prompt
echo "${YELLOW}[*] Digite a senha de root para instalação dos pacotes sem interação do usuário:${NYEL}"
read -s root_password

# Solicita o IP do servidor do coletor/syslog
echo "${YELLOW}[*] Digite o IP do servidor do coletor/syslog:${NYEL}"
read ip_collector

# Detecta a versão do SO
echo "${YELLOW}[*] Detectando versão do SO...${NYEL}"
...
case "$os_version" in
  "Ubuntu" | "Debian" | "CentOS" | "Red Hat Enterprise Linux (RHEL)")
    echo "[+] Detectado: $os_version"
    ;;
  *)
    echo "${RED}[!] Sistema não compatível com o instalador... Por favor, contate o suporte para instalação manual.${NRED}"
    exit 1
    ;;
esac

# Detecta a conexão com a internet
echo "${YELLOW}[*] Testando conexão com a internet...${NYEL}"
if ! ping -c 1 google.com &>/dev/null; then
  echo "${RED}[!] Sem conexão com a internet... Por favor, entre em contato com o suporte para instalação manual.${NRED}"
  exit 1
fi
...
if [ -n "$packages_to_install" ]; then
  echo "${YELLOW}[*] Os seguintes pacotes não estão instalados e precisam ser instalados:$packages_to_install${NYEL}"
  while true; do
    ...
    case "$install_choice" in
      [Yy]*)
        ...
        ;;
      [Nn]*)
        echo "[!] Instalação interrompida pelo usuário."
        exit 0
        ;;
      *)
...
if [ -f "/etc/rsyslog.conf" ]; then
  echo "[*] Backup do arquivo rsyslog.conf criado em /etc/rsyslog.conf.backup"
